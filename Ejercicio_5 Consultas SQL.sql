...
SELECT film_id, COUNT(actor_id) AS actorCount FROM film_actor
GROUP BY film_id ORDER BY actorCount DESC
LIMIT 1;

--17. ¿Qué actor (id) aparece en la mayor cantidad de películas?
SELECT actor_id, COUNT(film_id) AS filmCount FROM film_actor
GROUP BY actor_id ORDER BY filmCount DESC
LIMIT 1;

--18. Cuenta el número de ciudades para cada country_id en la tabla city. Ordena los resultados por count(*).
SELECT country_id, COUNT(city) as cityCount FROM city
GROUP BY country_id
ORDER BY cityCount;
